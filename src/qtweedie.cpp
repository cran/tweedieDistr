...
// Vectorised bisection root-finder for monotone CDF inversion.
static arma::vec solver_bisection(const arma::vec& q, const arma::vec& lambda,
  ...
  // Expand upper bound until F(x_U) >= q everywhere
  ...
  // Expand lower bound until F(x_L) <= q everywhere
...
// Vectorised Newton-Raphson root-finder; falls back to bisection on failure.
...
arma::vec tweedieInvCDF(arma::vec q, arma::vec mean, arma::vec dispersion,
  ...
  // q > exp(-lambda): solve on the positive axis
