...
// Vectorise R's scalar Poisson mass function
...
// Vectorise R's scalar Gamma cumulative distribution function
...
// Compute the compound Poisson-Gamma CDF
arma::vec compound_Poisson_Gamma(const arma::vec& x,
  ...
  while (!idx_low.is_empty()) {
    ...
    // Compute the log Poisson mass for the current active values
    ...
    // Update the CDF for the active values
  ...
  // Return the CDF constrained to the unit interval
